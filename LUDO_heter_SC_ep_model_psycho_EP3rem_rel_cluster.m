...
ts=vertcat(Bold_EP_stages{3},Bold_EP_stages{4});
NSUB = size(ts,1);
Nodes = size(ts,2);

load('all_STAGES_clean_ep_sc_scale1')
sc_ep=vertcat(permute (SC_EP_stages{3},[3,1,2]),permute (SC_EP_stages{4},[3,1,2]));
sc_ep=permute(sc_ep,[2,3,1]);
...
save(sprintf('model_SC_ep_hetero_EP3rem_trial_%02d',trial),'ksP','pcS','meta','Phases','xs','bifpar')
EOF
